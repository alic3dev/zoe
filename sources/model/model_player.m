...
void zoe_model_player_initialize(
  ...
  struct metil_joint* metil_joint_player_hip_left = &(
    metil_model->joints[
      zoe_model_player_joint_index_hip_left
    ]
  );

  struct metil_joint* metil_joint_player_hip_right = &(
    metil_model->joints[
      zoe_model_player_joint_index_hip_right
    ]
  );

  struct metil_joint* metil_joint_player_knee_left = &(
    metil_model->joints[
      zoe_model_player_joint_index_knee_left
    ]
  );

  struct metil_joint* metil_joint_player_knee_right = &(
    metil_model->joints[
      zoe_model_player_joint_index_knee_right
    ]
  );
  ...
  metil_joint_player_hip_left->position.x = (
    metil_object_leg_left->position.x
  );

  metil_joint_player_hip_left->position.y = (
    metil_object_leg_left->position.y +
    metil_object_leg_left->mesh.size.y /
    2.0f
  );

  metil_joint_player_hip_left->position.z = (
    metil_object_leg_left->position.z
  );

  metil_joint_player_hip_right->position.x = (
    metil_object_leg_right->position.x
  );

  metil_joint_player_hip_right->position.y = (
    metil_object_leg_right->position.y +
    metil_object_leg_right->mesh.size.y /
    2.0f
  );

  metil_joint_player_hip_right->position.z = (
    metil_object_leg_right->position.z
  );

  metil_joint_player_knee_left->position.x = (
    metil_object_leg_left->position.x
  );

  metil_joint_player_knee_left->position.y = (
    metil_object_leg_left->position.y
  );

  metil_joint_player_knee_left->position.z = (
    metil_object_leg_left->position.z
  );

  metil_joint_player_knee_right->position.x = (
    metil_object_leg_right->position.x
  );

  metil_joint_player_knee_right->position.y = (
    metil_object_leg_right->position.y
  );

  metil_joint_player_knee_right->position.z = (
    metil_object_leg_right->position.z
  );
  ...
  for (
    unsigned int index_vertex = 0;
    index_vertex < metil_object_leg_left->mesh.length_vertices;
    ++index_vertex
  ) {
    if (
      index_vertex < (
        metil_object_leg_left->mesh.length_vertices /
        2.0f
      )
    ) {
      metil_model_vertex_joint_attach(
  ...
  for (
    unsigned int index_vertex = 0;
    index_vertex < metil_object_leg_right->mesh.length_vertices;
    ++index_vertex
  ) {
    if (
      index_vertex < (
        metil_object_leg_right->mesh.length_vertices /
        2.0f
      )
    ) {
      metil_model_vertex_joint_attach(
...
void zoe_model_player_poll(
  ...
  metil_joint_player_hip_left->rotation.x = (
    rotation_hip_x
  );

  metil_joint_player_hip_right->rotation.x = (
    rotation_hip_x
  );

  metil_joint_player_knee_left->rotation.x = (
    rotation_knee_x
  );

  metil_joint_player_knee_right->rotation.x = (
    rotation_knee_x
  );
